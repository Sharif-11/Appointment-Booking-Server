...
const createDoctorService = userData =>
  __awaiter(void 0, void 0, void 0, function* () {
    const role = 'Doctor'
    const { phoneNo, password } = userData,
      doctor = __rest(userData, ['phoneNo', 'password'])
    console.log({ doctor })
    const user = { phoneNo, password, role }
    const session = yield mongoose_1.default.startSession()
    try {
      session.startTransaction()
      const newDoctor = yield doctor_model_1.default.create([doctor], {
        session,
      })
      if (!newDoctor.length) {
